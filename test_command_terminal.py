...
# -*- coding: utf-8 -*-
"""
Command/Terminal实体、仓库、DTO三合一自动化测试脚本
统一使用 cloudcontrol_test 数据库
严格禁用H2，仅依赖V1~V4迁移，测试外键完整性
"""
...
def clear_flyway_migration():
    ...
    "'''
    run_command(drop_tables_cmd, f"删除 {db} 的所有相关表")
...
def run_command_terminal_tests():
    print("\n=== 运行Command/Terminal实体、仓库、DTO测试 ===")
    test_commands = [
        "cd backend && mvn clean compile -DskipTests",
        # 先测Terminal
        "cd backend && mvn test -Dtest=TerminalRepositoryOnlyTest",
        # 再测Command（需保证Terminal已存在）
        "cd backend && mvn test -Dtest=CommandRepositoryTest"
    ]
...
def main():
    print("=" * 60)
    print("Command/Terminal实体、仓库、DTO三合一测试")
    ...
    required_files = [
        ...
        "backend/src/test/java/com/cloudcontrol/repository/command/CommandRepositoryTest.java"
    ]
